...
$BLIBOX_VERSION = 'v1.7.0';
$BLIBOX_DATE = '2020-03-24';
$BLIBOX_API_PAGE = 'blibox-api/status.json';
...
$LOG_DIR	= dirname(dirname($CONF_DIR)) . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'blibox';
	...
	if (isset($_LOADED_LIBS[$class])) {
		...
		switch($class) {
			...
			case 'Logger':
				...
				$_LOADED_LIBS[$class] = \blibox\Logger::getInstance();
				break;

			case 'Html':
				loadFile($class, $lib_dir);
				$_LOADED_LIBS[$class] = \blibox\Html::getInstance();
				break;

			case 'Helper':
				loadFile($class, $lib_dir);
				$_LOADED_LIBS[$class] = \blibox\Helper::getInstance();
				break;
			...
			case 'Php':
				...
				$_LOADED_LIBS[$class] = \blibox\Php::getInstance($GLOBALS['PHP_HOST_NAME']);
				break;

			case 'Dns':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \blibox\Dns::getInstance($GLOBALS['DNS_HOST_NAME']);
				break;

			case 'Httpd':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \blibox\Httpd::getInstance($GLOBALS['HTTPD_HOST_NAME']);
				break;

			case 'Mysql':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \blibox\Mysql::getInstance($GLOBALS['MYSQL_HOST_NAME'], array(
					'user' => 'root',
			...
			case 'Pgsql':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \blibox\Pgsql::getInstance($GLOBALS['PGSQL_HOST_NAME'], array(
			...
			case 'Redis':
				...
				if ($REDIS_ROOT_PASSWORD == '') {
					$_LOADED_LIBS[$class] = \blibox\Redis::getInstance($GLOBALS['REDIS_HOST_NAME']);
				} else {
					$_LOADED_LIBS[$class] = \blibox\Redis::getInstance($GLOBALS['REDIS_HOST_NAME'], array(
			...
			case 'Memcd':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \blibox\Memcd::getInstance($GLOBALS['MEMCD_HOST_NAME']);
				break;

			case 'Mongo':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \blibox\Mongo::getInstance($GLOBALS['MONGO_HOST_NAME']);
				break;
