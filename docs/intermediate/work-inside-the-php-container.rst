...
The Blibox allows you to completely work inside the PHP container (no matter what version),
...
Entering the computer is fairly simple. The Blibox ships with two scripts to do that. One for
...
.. code-block:: bash
   ...
   blibox@php-7.0.19 in /shared/httpd $
...
.. code-block:: bash
   ...
   # Navigate to the Blibox directory
   C:/> cd C:/Users/user1/blibox

   # Run provided script
   C:/Users/user1/blibox> shell.bat
...
By using the provided scripts to enter the container you will become the user ``blibox``.
...
.. code-block:: bash
   ...
   # As user blibox inside the container
   blibox@php-7.0.19 in /shared/httpd $ sudo su -
...
.. code-block:: bash
   ...
   # As user blibox inside the container
   blibox@php-7.0.19 in /shared/httpd $ sudo apt update
   blibox@php-7.0.19 in /shared/httpd $ sudo apt install nmap
...
.. code-block:: bash
   ...
   # As user blibox inside the container
   blibox@php-7.0.19 in /shared/httpd $ exit
...
The username inside the container (``blibox``) might be different from your local host operating
...
.. code-block:: bash
   ...
   blibox@php-7.0.19 in /shared/httpd $ mysql -h mysql -u root -p
