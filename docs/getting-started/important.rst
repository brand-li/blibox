...
to use the Blibox and Docker in general.
...
Do not start the Blibox with ``sudo`` or as ``root`` user. If it complains about permissions when
...
.. seealso:: **Ensure you have read and done the following:**
   ...
   * Synronize file permissions: :ref:`install_the_blibox_set_uid_and_gid`

.. warning:: If you start the Blibox with ``sudo`` or as ``root`` user, it will most likely mess with your file permissions.


Starting, Stopping and Restarting
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Whenever you want to stop the Blibox, change configuration and start up again, do not forget to
