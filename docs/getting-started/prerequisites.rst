...
The Blibox runs on all major operating systems which provide ``Docker`` and ``Docker Compose``.
...
The only requirements for the Blibox is to have ``Docker`` and ``Docker Compose`` installed,
...
Additionally you will require ``git`` in order to clone the blibox project.
...
.. important:: :ref:`howto_docker_toolbox_and_the_blibox`
...
the Blibox. This however is only available since **Windows 10**.
...
In order to easily work with the Blibox you should already be familiar with the following:
