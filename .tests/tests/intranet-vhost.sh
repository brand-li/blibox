...
run "sleep 4"
run "docker-compose exec --user blibox -T php mkdir -p /shared/httpd/${VHOST}" "${RETRIES}" "${BLIBOX_PATH}"
run "sleep 4"
...
###
run "docker-compose exec --user blibox -T php mkdir -p /shared/httpd/${VHOST}/${HTTPD_DOCROOT_DIR}" "${RETRIES}" "${BLIBOX_PATH}"
...
if ! run "curl -sS --fail --header 'host: ${VHOST}.${TLD_SUFFIX}' 'http://localhost:${HOST_PORT_HTTPD}/blibox-api/status.json' | tac | tac | grep 'success' >/dev/null" "${RETRIES}" "" "0"; then
