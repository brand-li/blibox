...
This is achieved in the Blibox's containers by two environment variables that can be provided
during startup in order to change the uid/gid of the PHP-FPM user prior starting up PHP-FPM process.

.. code-block:: bash

   $ docker run -e NEW_UID=1000 -e NEW_GID=1000 -it blibox/php-fpm:7.2-work
   [INFO] Changing user 'blibox' uid to: 1000
   root $ usermod -u 1000 blibox
   [INFO] Changing group 'blibox' gid to: 1000
   root $ groupmod -g 1000 blibox
