...
PHP_TAG="$( grep 'blibox/php' "${BLIBOX_PATH}/docker-compose.yml" | sed 's/^.*-work-//g' )"
...
###
if ! PHP52_BASE="$( run "\
	curl -sS 'https://raw.githubusercontent.com/blibox/docker-php-fpm/${PHP_TAG}/README.md' \
	| tac \
	| tac \
	| grep -E '52-base' \
	| sed \
...
###
DISABLED=",blackfire,ioncube,$( grep -E '^PHP_MODULES_DISABLE=' "${BLIBOX_PATH}/env-example" | sed 's/.*=//g' ),"
#echo $DISABLED
