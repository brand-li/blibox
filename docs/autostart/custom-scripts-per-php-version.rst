...
.. code-block:: bash
   ...
   host> ls -l path/to/blibox/cfg/ | grep 'php-startup'
...
scripts, ensure to restart the Blibox.
...
.. code-block:: bash
   ...
   host> cd path/to/blibox/cfg/php-startup-7.1
...
Running commands as blibox user
---------------------------------

As mentioned above, all scripts are run by the ``root`` user.
If you do need something to be executed as the normal user: ``blibox``, you can simply ``su``
inside the shell script.

The following example will install ``grunt`` and start a NodeJS application as the blibox user
for the PHP 7.1 Docker container only.

.. code-block:: bash
   :caption: cfg/php-startup-7.1/myscript.sh

   # Install grunt as blibox user
   su -c "npm install grunt" -l blibox

   # Start a NodeJS application with pm2 as blibox user
   su -c "cd /shared/httpd/my-node/src/; pm2 start index.js" -l blibox
