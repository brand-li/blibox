...
class Helper
{
	...
	{
		$dvl_password = loadClass('Helper')->getEnv('BLIBOX_UI_PASSWORD');

		if ($username == 'blibox' && $password == $dvl_password) {
			$_SESSION['auth'] = 1;
			return true;
		}
		return false;
	...
	public function isLoginProtected()
	{
		...
		if (loadClass('Helper')->getEnv('BLIBOX_UI_PROTECT') != 1) {
			return false;
		}
		return true;
