...
Imagine you have added a custom service container to the Blibox which has a project that
...
via the bundled web server and also be able to see it on the Blibox intranet vhost section.
...
Let's imagine you have added a custom Python Docker image to the Blibox which starts up a Django
application listening on port ``3000``.

* :ref:`env_TLD_SUFFIX`: ``loc``
* Desired DNS name: ``my-python.loc``
* :ref:`env_httpd_datadir` on the host: ``./data/www``
* :ref:`env_httpd_template_dir`: ``.blibox``
* Listening port: ``3000``
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox/data/www
   ...
   host> mkdir my-python/.blibox

This part is now sufficient to have the project visible on the Blibox intranet.
...
For this example we will copy all ``*-example-rproxy`` files into ``/shared/httpd/my-python/.blibox``
to ensure this will work with all web servers.

.. code-block:: bash

   host> cd /path/to/blibox
   host> cp cfg/vhost-gen/apache22.yml-example-rproxy data/www/my-python/.blibox/apache22.yml
   host> cp cfg/vhost-gen/apache24.yml-example-rproxy data/www/my-python/.blibox/apache24.yml
   host> cp cfg/vhost-gen/nginx.yml-example-rproxy data/www/my-python/.blibox/nginx.yml
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-python/.blibox/apache24.yml

Find the two lines with ``ProxyPass`` and ``ProxyPassReverse`` and change the port from ``8000``
to ``3000`` and host ``php`` to ``python``:

.. code-block:: yaml
   :caption: data/www/my-python/.blibox/apache24.yml
   :emphasize-lines: 16,17
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-python/.blibox/nginx.yml

Find the line with ``proxy_pass`` and change the port from ``8000``
to ``3000`` and host ``php`` to ``python``:

.. code-block:: yaml
   :caption: data/www/my-python/.blibox/nginx.yml
   :emphasize-lines: 18
...
Now for the changes to take affect, simply restart the Blibox (or start if not yet running):
...
.. code-block:: bash
   ...
   # Start the Blibox (Your Python container and the PHP and HTTPD container only)
...
And is even shown as a project in the Blibox intranet:
