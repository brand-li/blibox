...
# bound to your blibox git state.
#
# When updating the blibox via git, do run this script once
...
if [ "${WHICH}" = "all" ] || [ "${WHICH}" = "php" ]; then
	SUFFIX="$( grep -E '^\s+image:\s+blibox/php-fpm' "${CWD}/docker-compose.yml" | sed 's/.*}//g' )"
	IMAGES="$( grep -Eo '^#*PHP_SERVER=[.0-9]+' "${CWD}/env-example" | sed 's/.*=//g' )"
	echo "${IMAGES}" | while read version ; do
		docker pull blibox/php-fpm:${version}${SUFFIX}
	done
fi
...
if [ "${WHICH}" = "all" ] || [ "${WHICH}" = "httpd" ]; then
	SUFFIX="$( grep -E '^\s+image:\s+blibox/\${HTTPD_SERVER' "${CWD}/docker-compose.yml" | sed 's/.*://g' )"
	IMAGES="$( grep -Eo '^#*HTTPD_SERVER=[-a-z]+[.0-9]*' "${CWD}/env-example" | sed 's/.*=//g' )"
	echo "${IMAGES}" | while read version ; do
		docker pull blibox/${version}:${SUFFIX}
	done
fi
...
if [ "${WHICH}" = "all" ] || [ "${WHICH}" = "mysql" ]; then
	...
	echo "${IMAGES}" | while read version ; do
		docker pull blibox/mysql:${version}
	done
fi
