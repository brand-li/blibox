...
TEST_REPO="https://github.com/blibox/docker-php-fpm"
TEST_PATH="tests/mods/modules"

# Get current PHP_FPM git tag or branch
PHP_FPM_GIT_SLUG="$( \
	grep -E '^[[:space:]]+image:[[:space:]]+blibox/php-fpm:' "${BLIBOX_PATH}/docker-compose.yml" \
...
run "docker-compose exec -T --user blibox php rm -rf /shared/httpd/${VHOST} || true" "${RETRIES}" "${BLIBOX_PATH}"
run "docker-compose exec -T --user blibox php mkdir -p /shared/httpd/${VHOST}" "${RETRIES}" "${BLIBOX_PATH}"
run "docker-compose exec -T --user blibox php svn checkout ${SVN_PATH} /shared/httpd/${VHOST}/htdocs" "${RETRIES}" "${BLIBOX_PATH}"
