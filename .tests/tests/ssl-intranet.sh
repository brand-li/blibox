...
if ! run "curl -sS --fail --cacert ${BLIBOX_PATH}/ca/blibox-ca.crt 'https://localhost:${HOST_PORT_HTTPD_SSL}' >/dev/null" "${RETRIES}" "" "0"; then
	printf "\\r[FAIL] https Intranet / from host\\n"
	run "curl -v --cacert ${BLIBOX_PATH}/ca/blibox-ca.crt 'https://localhost:${HOST_PORT_HTTPD_SSL}' || true" "1"
	ERROR=1
else
...
if ! run "docker-compose exec -T php curl -sS --fail 'https://httpd' >/dev/null" "${RETRIES}" "${BLIBOX_PATH}" "0"; then
	printf "\\r[FAIL] https Intranet / from container\\n"
	run "docker-compose exec -T php curl -v 'https://httpd' || true" "1" "${BLIBOX_PATH}"
	ERROR=1
else
...
if ! run "curl -sS --fail --cacert ${BLIBOX_PATH}/ca/blibox-ca.crt 'https://localhost:${HOST_PORT_HTTPD_SSL}/credits.php' | tac | tac | grep -E 'https:\\/\\/github\\.com\\/cytopia' >/dev/null" "${RETRIES}" "" "0"; then
	printf "\\r[FAIL] https Intranet /credits.php from host\\n"
	run "curl -v --cacert ${BLIBOX_PATH}/ca/blibox-ca.crt 'https://localhost:${HOST_PORT_HTTPD_SSL}/credits.php' || true" "1"
	ERROR=1
else
...
if ! run "docker-compose exec -T php curl -sS --fail 'https://httpd/credits.php' | tac | tac | grep -E 'https:\\/\\/github\\.com\\/cytopia' >/dev/null" "${RETRIES}" "${BLIBOX_PATH}" "0"; then
	printf "\\r[FAIL] https Intranet /credits.php from container\\n"
	run "docker-compose exec -T php curl -v 'https://httpd/credits.php' || true" "1" "${BLIBOX_PATH}"
	ERROR=1
else
