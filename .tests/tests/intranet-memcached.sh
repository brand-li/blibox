...
printf "[TEST] blibox-version key in Memcached"
if  ! run "curl -sS --fail 'http://localhost:${HOST_PORT_HTTPD}/db_memcd.php' | tac | tac | grep 'blibox-version' >/dev/null" "${RETRIES}" "" "0"; then
	printf "\\r[FAIL] blibox-version key in Memcached\\n"
	run "curl 'http://localhost:${HOST_PORT_HTTPD}/db_memcd.php' || true"
	exit 1
else
	printf "\\r[OK]   blibox-version key in Memcached\\n"
fi
