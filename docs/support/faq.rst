...
Yes, read :ref:`howto_docker_toolbox_and_the_blibox` to find out more.
...
This is just a pre-caution. Imagine they would link to the same datadir.  You start the Blibox
with mysql 5.5, create a database and add some data.  Now you decide to switch to mysql 5.7 and
restart the blibox. The newer mysql version will probably upgrade the data leaving it unable to
...
up the blibox, the PHP container will use these values for its user.
...
The ``.env`` file allows you to configure the blibox, but not to start services selectively.
...
The Blibox is a development stack, so it is made sure that a lot of PHP modules are available
...
.. seealso:: https://github.com/blibox/docker-php-fpm
...
Yes, every Docker container is optional. The Blibox allows for selective startup. See
:ref:`start_the_blibox`.
...
don't exists by default, you will have to create them. However, the Blibox has a bundled DNS
