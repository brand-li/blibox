...
separated compressed file. Backups will be located in ``./backups/mysql/`` inside the Blibox
...
.. code-block:: bash
   ...
   blibox@php-7.1.6 in /shared/httpd $ mysqldump-secure
...
.. code-block:: bash
   ...
   blibox@php-7.1.6 in /shared/httpd $ ls -l /shared/backups/mysql/

   -rw-r--r-- 1 blibox 136751 Jun 17 13:31 2017-06-17_13-31__mysql.sql.gz
   -rw-r--r-- 1 blibox   2269 Jun 17 13:31 2017-06-17_13-31__mysql.sql.gz.info


Let's do the same again and see where to find the backups in the Blibox git directory
...
.. code-block:: bash
   ...
   # Start the backup
   blibox@php-7.1.6 in /shared/httpd $ mysqldump -h mysql -u root -p my_db_name > /shared/backups/mysql/my_db_name.sql
...
It comes bundled with the blibox intranet.
...
.. code-block:: bash
   ...
   # Start the import
   blibox@php-7.1.6 in /shared/httpd $ mysql -h mysql -u root -p my_db_name < /shared/backups/mysql/my_db_name.sql
...
.. code-block:: bash
   ...
   # Start the import
   blibox@php-7.1.6 in /shared/httpd $ zcat /shared/backups/mysql/my_db_name.sql.gz | mysql -h mysql -u root -p my_db_name
...
.. code-block:: bash
   ...
   # Start the import
   blibox@php-7.1.6 in /shared/httpd $ tar xzOf /shared/backups/mysql/my_db_name.sql.tar.gz | mysql -h mysql -u root -p my_db_name
