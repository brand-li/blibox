...
This section explains how to connect from inside a Blibox container to the host operating system.
...
IP address within the Blibox bridge network (see ``docker-compose.yml``).
...
or Windows, therefore two custom CNAME's have been added by the Blibox in order to emulate the
same behaviour:
...
The Blibox IP address or the custom provided CNAMEs actually refer to the Docker Toolbox machine.
...
* Blibox Docker container wants to access MySQL on host os
...
to that service from within the Blibox Docker container.
