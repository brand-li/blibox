...
The Blibox startup explained
==============================

To gain a brief understanding about what is happening under the hood during startup,
read ahead or skip directly to: :ref:`start_the_blibox_start_all_container`.
...
.. note::
   ...
   If you want to update to new Docker images read on: :ref:`update_the_blibox`.


.. _start_the_blibox_start_all_container:
...
.. _start_the_blibox_stop_and_restart:
...
.. important::
   ...
   When stopping or restarting the Blibox, ensure to also **remove stopped container** before the
...
Open Blibox intranet
======================

Once ``docker-compose up`` has finished and all or the selected container are up and running,
you can visit the Blibox intranet with your favorite Web browser at http://localhost or
...
.. include:: /_includes/figures/blibox/blibox-intranet-dash-all.rst
.. include:: /_includes/figures/blibox/blibox-intranet-dash-selective.rst

.. important::
   :ref:`howto_find_docker_toolbox_ip_address`
      When you are using ``Docker Toolbox`` the Blibox web server port will not be available on
...
2. ``docker-compose rm -f`` is issued before restarting the Blibox
