...
If this file does not exist at the root of your Blibox git directory, then copy ``env-example``
...
.. important::
   Any change of ``.env`` requires a restart of the Blibox.
...
under ``./log/`` inside the Blibox git directory or they can be forwarded to Docker logs
...
.. note::
   Log directories do not exist until you start the Blibox and will only be created for
...
This specifies a relative or absolute path to the Blibox git directory and will be used as a
prefix for all Docker mount paths.

* Relative path: relative to the blibox git directory (Must start with ``.``)
...
.. warning::
   :ref:`remove_stopped_container`
     Whenever you change this value you have to stop the Blibox and also remove the stopped
     container via
     ``docker-compose rm``.
...
.. warning::
   **Do not use official domain endings** such as ``.com``, ``.org``, ``.net``, etc.
   ...
   to the Blibox's PHP server IP address.
...
This variable allows you to add additional DNS entries from hosts outside the Blibox network,
...
This setting controls one of the core concepts of the Blibox. It overcomes the problem of
...
The Blibox own containers will then pick up this value during startup and change their internal
...
This variable controls the system as well as service timezone for the Blibox's own containers.
...
The Blibox intranet validates if every project has a corresponding DNS record (either an official
...
.. _env_blibox_ui_ssl_cn:

BLIBOX_UI_SSL_CN
------------------

When accessing the Blibox intranet via ``https`` it will use an automatically created SSL certificate.
Each SSL certificate requires a valid Common Name, which must match the virtual host name.

This setting let's you specify by what **name** you are accessing the Blibox intranet.
...
| Name                    | Allowed values               | Default value                                 |
+=========================+==============================+===============================================+
| ``BLIBOX_UI_SSL_CN``  | comma separated list of CN's | ``localhost,*.localhost,blibox,*.blibox`` |
+-------------------------+------------------------------+-----------------------------------------------+

**Examples**:

* ``BLIBOX_UI_SSL_CN=localhost``
* ``BLIBOX_UI_SSL_CN=localhost,*.localhost``
* ``BLIBOX_UI_SSL_CN=localhost,*.localhost,blibox,*.blibox``
* ``BLIBOX_UI_SSL_CN=intranet.example.com``

.. seealso:: :ref:`setup_valid_https`


.. _env_blibox_ui_protect:

BLIBOX_UI_PROTECT
-------------------

By setting this variable to ``1``, the Blibox intranet will be password protected.
This might be useful, if you share your running Blibox instance accross a LAN, but do not want
everybody to have access to the intranet itself, just to the projects you actually provide.

+-------------------------+----------------+-------------------+
| Name                    | Allowed values | Default value     |
+=========================+================+===================+
| ``BLIBOX_UI_PROTECT`` | ``0`` or ``1`` | ``0``             |
+-------------------------+----------------+-------------------+
...
.. _env_blibox_ui_password:

BLIBOX_UI_PASSWORD
--------------------

When the blibox intranet is password-protected via ``BLIBOX_UI_PROTECT``, this is the actual
password by which it will be protected.

+--------------------------+----------------+-------------------+
| Name                     | Allowed values | Default value     |
+==========================+================+===================+
| ``BLIBOX_UI_PASSWORD`` | any string     | ``password``      |
+--------------------------+----------------+-------------------+


.. _env_blibox_ui_enable:

BLIBOX_UI_ENABLE
------------------

In case you want to completely disable the Blibox intranet, such as when running it on production,
...
| Name                    | Allowed values | Default value     |
+=========================+================+===================+
| ``BLIBOX_UI_ENABLE``  | ``0`` or ``1`` | ``1``             |
+-------------------------+----------------+-------------------+


BLIBOX_VENDOR_PHPMYADMIN_AUTOLOGIN
------------------------------------
...
| Name                                      | Allowed values | Default value     |
+===========================================+================+===================+
| ``BLIBOX_VENDOR_PHPMYADMIN_AUTOLOGIN``  | ``0`` or ``1`` | ``1``             |
+-------------------------------------------+----------------+-------------------+


BLIBOX_VENDOR_PHPPGADMIN_AUTOLOGIN
------------------------------------
...
| Name                                      | Allowed values | Default value     |
+===========================================+================+===================+
| ``BLIBOX_VENDOR_PHPPGADMIN_AUTOLOGIN``  | ``0`` or ``1`` | ``1``             |
+-------------------------------------------+----------------+-------------------+
...
The following settings reflect one of the main goals of the Blibox: being able to run any
combination of all container versions.

.. note::
   Any change for those settings requires a restart of the blibox.
...
.. important::
   **PHP 5.2** is available to use, but it is not officially supported. The Blibox intranet does
...
This is an absolute or relative path (relative to Blibox git directory) to your data directory.
...
to separate your data from the Blibox git directory, do change the path to a place where you
...
PHP container into ``/home/blibox/.ssh``.
...
.. warning::
   :ref:`howto_docker_toolbox_and_the_blibox`
...
.. warning::
   ...
   Blibox. When doing so, read this article with care: :ref:`setup_auto_dns`.
...
When set to ``2`` (email catch-all) postfix is started, but no mail will leave the Blibox. It is automatically
internally routed the the blibox mail account and you can see each sent mail
...
* blibox git directory location: ``/home/user-1/repo/blibox``
...
``/home/user-1/repo/blibox/data/www/my-first-project/htdocs``
...
| Name                   | Allowed values    | Default value    |
+========================+===================+==================+
| ``HTTPD_TEMPLATE_DIR`` | valid dir name    | ``.blibox``    |
+------------------------+-------------------+------------------+
...
``.blibox`` by default. If you change the ``HTTPD_TEMPLATE_DIR`` variable to something else,
...
.. code-block:: bash
   ...
   host> ls -l
   total 4
   drwxr-xr-x 2 cytopia cytopia 4096 Mar 12 23:05 .blibox/
   drwxr-xr-x 2 cytopia cytopia 4096 Mar 12 23:05 htdocs/

Now you need to copy the ``vhost-gen`` templates into the ``.blibox`` directory. The templates
are available in the Blibox git directory under ``cfg/vhost-gen/``.
...
.. code-block:: bash
   ...
   # Navigate into the blibox directory
   host> cd path/to/blibox

   # Copy templates to your project directory
   host> cp cfg/vhost-gen/*.yml data/www/my-first-project/.blibox/
...
.. code-block:: bash
   ...
   host> ls -l data/www/my-first-project/htdocs/.blibox
   total 4
...
accordingly in ``.env``, otherwise the blibox will not be able to connect to MySQL and will not
...
make sure to also set it accordingly in .``env,`` otherwise the blibox will not be able to
...
The blibox is using its own DNS server internally (your host computer can also use it for
