...
Besides providing basic LAMP/MEAN stack container, which are well integrated into the Blibox
intranet, the Blibox also ships additional pre-configured container that can easily be enabled.
...
Copy ``docker-compose.override.yml-all`` into the root of the Blibox git directory.
