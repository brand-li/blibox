...
.. note::
   It is also possible to attach a leight-weight Python container to the Blibox instead of running
...
.. code-block:: bash
   ...
   blibox@php-7.0.20 in /shared/httpd $ mkdir my-sphinx
...
.. code-block:: bash
   ...
   blibox@php-7.0.20 in /shared/httpd $ cd my-sphinx

   # Create a directory which will hold the documentation source code
   blibox@php-7.0.20 in /shared/httpd/my-sphinx $ mkdir doc
...
.. code-block:: bash
   ...
   blibox@php-7.0.20 in /shared/httpd/my-sphinx $ mkdir htdocs
...
.. code-block:: bash
   ...
   blibox@php-7.0.20 in /shared/httpd/my-sphinx $ mkdir .blibox
...
For this example we will copy all ``*-example-rproxy`` files into ``/shared/httpd/my-sphinx/.blibox``
to ensure this will work with all web servers.

.. code-block:: bash

   host> cd /path/to/blibox
   host> cp cfg/vhost-gen/apache22.yml-example-rproxy data/www/my-sphinx/.blibox/apache22.yml
   host> cp cfg/vhost-gen/apache24.yml-example-rproxy data/www/my-sphinx/.blibox/apache24.yml
   host> cp cfg/vhost-gen/nginx.yml-example-rproxy data/www/my-sphinx/.blibox/nginx.yml
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-sphinx/.blibox/apache22.yml
...
.. code-block:: yaml
   :caption: data/www/my-sphinx/.blibox/apache22.yml
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-sphinx/.blibox/apache24.yml
...
.. code-block:: yaml
   :caption: data/www/my-sphinx/.blibox/apache24.yml
   :emphasize-lines: 16,17
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-sphinx/.blibox/nginx.yml


Find the lines with ``proxy_pass`` and change the port from ``8000`` to ``4000``

.. code-block:: yaml
   :caption: data/www/my-sphinx/.blibox/nginx.yml
   :emphasize-lines: 18
...
Navigate to ``cfg/php-startup-7.2/`` in the Blibox git directory and create a new shell script
ending by ``.sh``


.. code-block:: bash

   # Navigate to the Blibox git directory
   host> cd /path/to/blibox
...
.. code-block:: bash
   ...
   # Autostart Sphinx by blibox user on Port 4000 and sent it to backgroun with '&'
   sh -c "cd /shared/httpd/my-sphinx; sphinx-autobuild . _build/html -p 4000 -H 0.0.0.0" -l blibox &
