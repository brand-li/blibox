...
Inside the Blibox git directory you will find a directory called ``bash/``.
...
The ``bash/`` directory will be mounted into the PHP container to ``/etc/bashrc-blibox.d/``.

+--------------+-----------------------------+
| Host OS path | Docker path                 |
+==============+=============================+
| ``./bash/``  | ``/etc/bashrc-blibox.d/`` |
+--------------+-----------------------------+
...
The ``.vimrc`` is usually place directly in the users home directory and the Blibox does not
...
``/etc/bashrc-blibox.d/.vimrc`` inside the PHP container, so just create a shell alias for vim
...
.. code-block:: bash
   ...
   alias vim='vim -u /etc/bashrc-blibox.d/.vimrc'
