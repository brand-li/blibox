...
.. _shared_blibox_server_in_lan:

*****************************
Shared Blibox server in LAN
*****************************

Blibox as a shared **development**, **staging** or **CI** server is setup in a similar way as
...
+--------------------+------------------+-----------+---------------------------+
| LAN / Network      | Blibox server  | TLD_SUFFX | LOCAL_LISTEN_ADDR         |
+====================+==================+===========+===========================+
...
^^^^^^^^^^^^^^^^^^^^^^^^^^
When using git, users can directly ssh into the shared Blibox server and ``git pull`` or
...
+---------------------+------------------+--------------------------------------------------------------------------------------------------------+
| Blibox DNS server | Manual           | Every network device must configure its DNS settings                                                   |
|                     +------------------+--------------------------------------------------------------------------------------------------------+
...
.. important::
   When using a shared Blibox server and another Blibox setup on your local computer,
...
Run a second instance of the Blibox DNS server
------------------------------------------------

If the above two methods for automated DNS records don't apply to you, you will need to run
a second stand-alone Docker container of the Blibox DNS server.

Run this container permantently on the shared Blibox server with the following command:
...
to access projects on the shared Blibox server.


Self-managed DHCP server
""""""""""""""""""""""""
If you run your own DHCP server within a network, you probably know how to add other DNS servers.
The only thing you should keep in mind is, that the Blibox DNS server should be the first in
the list.
...
in its web interface and add the Blibox DNS server as the first in the list (``192.168.0.12``).
...
As you also do for the Blibox locally when not using Auto-DNS, you can do as well for remote
...
The last step to also have valid HTTPS connections on your shared Blibox server is to copy
