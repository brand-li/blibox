...
.. note::
   It is also possible to attach a leight-weight NodeJS container to the Blibox instead of running
...
.. note::
   * Inside the Blibox PHP container, projects are always in ``/shared/httpd/``.
   * On your host operating system, projects are by default in ``./data/www/`` inside the
     Blibox git directory. This path can be changed via :ref:`env_httpd_datadir`.
...
Navigate to the Blibox git directory and execute ``shell.sh`` (or ``shell.bat`` on Windows) to
...
.. code-block:: bash
   ...
   blibox@php-7.0.20 in /shared/httpd $ mkdir my-node
...
.. code-block:: bash
   ...
   blibox@php-7.0.20 in /shared/httpd $ cd my-node

   # Create a directory which will hold the source code
   blibox@php-7.0.20 in /shared/httpd/my-node $ mkdir src

   # Create the index.js file with your favourite editor
   blibox@php-7.0.20 in /shared/httpd/my-node/src $ vi index.js
...
Every project for the Blibox requires a ``htdocs`` directory present inside the project dir.
...
.. code-block:: bash
   ...
   blibox@php-7.0.20 in /shared/httpd/my-node $ mkdir htdocs
...
Before we can copy the vhost-gen templates, we must create the ``.blibox`` template directory
...
.. code-block:: bash
   ...
   # Create the .blibox template directory
   blibox@php-7.0.20 in /shared/httpd/my-node $ mkdir .blibox
...
For this example we will copy all ``*-example-rproxy`` files into ``/shared/httpd/my-node/.blibox``
to ensure this will work with all web servers.

.. code-block:: bash

   host> cd /path/to/blibox
   host> cp cfg/vhost-gen/apache22.yml-example-rproxy data/www/my-node/.blibox/apache22.yml
   host> cp cfg/vhost-gen/apache24.yml-example-rproxy data/www/my-node/.blibox/apache24.yml
   host> cp cfg/vhost-gen/nginx.yml-example-rproxy data/www/my-node/.blibox/nginx.yml
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-node/.blibox/apache22.yml
...
.. code-block:: yaml
   :caption: data/www/my-node/.blibox/apache22.yml
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-node/.blibox/apache24.yml
...
.. code-block:: yaml
   :caption: data/www/my-node/.blibox/apache24.yml
   :emphasize-lines: 16,17
...
.. code-block:: bash
   ...
   host> cd /path/to/blibox
   host> vi data/www/my-node/.blibox/nginx.yml


Find the lines with ``proxy_pass`` and change the port from ``8000`` to ``4000``

.. code-block:: yaml
   :caption: data/www/my-node/.blibox/nginx.yml
   :emphasize-lines: 18
...
For NodeJS applications, the Blibox already bundles an autostart template which you can use
...
So in order to proceed copy this file inside the ``autostart/`` directory of the Blibox git directory
...
Now for those changes to take affect, you will have to restart the Blibox.
...
.. code-block:: bash
   ...
   # Navigate to Blibox git directory
   host> cd /path/to/blibox
   ...
   blibox@php-7.0.20 in /shared/httpd $ pm2 list

   ┌──────────┬────┬─────────┬──────┬──────┬────────┬─────────┬────────┬─────┬───────────┬──────────┬──────────┐
   │ App name │ id │ version │ mode │ pid  │ status │ restart │ uptime │ cpu │ mem       │ user     │ watching │
   ├──────────┼────┼─────────┼──────┼──────┼────────┼─────────┼────────┼─────┼───────────┼──────────┼──────────┤
   │ index    │ 0  │ N/A     │ fork │ 1906 │ online │ 0       │ 42h    │ 0%  │ 39.7 MB   │ blibox │ disabled │
   └──────────┴────┴─────────┴──────┴──────┴────────┴─────────┴────────┴─────┴───────────┴──────────┴──────────┘
