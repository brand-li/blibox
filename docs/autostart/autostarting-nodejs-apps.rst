...
* The whole command is wrapped into ``su`` to ensure the application will be started as the user ``blibox``.
...
Once the Blibox is running, you can enter the PHP container and verify with ``pm2 list`` that
