...
The latest version of vhost-gen templates are shipped in the Blibox git directory under
``cfg/vhost-gen/``.
...
3. Adjust template
4. Make Blibox pick up those changes
...
| Variable                      | Value                                                 |
+===============================+=======================================================+
| Blibox path                 | ``/home/user/blibox``                               |
+-------------------------------+-------------------------------------------------------+
| Templates to copy from        | ``/home/user/blibox/cfg/vhost-gen``                 |
+-------------------------------+-------------------------------------------------------+
| Project name                  | ``project-1``                                         |
+-------------------------------+-------------------------------------------------------+
| :ref:`env_httpd_template_dir` | ``.blibox`` (default value)                         |
+-------------------------------+-------------------------------------------------------+
...
| What                          | Path                                                  |
+===============================+=======================================================+
| Project directory path        | ``/home/user/blibox/data/www/project-1/``           |
+-------------------------------+-------------------------------------------------------+
| Project template path         | ``/home/user/blibox/data/www/project-1/.blibox/`` |
+-------------------------------+-------------------------------------------------------+

1. Retrieve or set template directory value
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

By default the :ref:`env_httpd_template_dir` value is ``.blibox``. This is defined in the
...
For this example we will keep the default value for the sake of simplicity: ``.blibox``.
...
.. code-block:: bash
   ...
   # Navigate to the Blibox directory
   host> cd /home/user/blibox

   # Create template directory in your project
   host> mkdir ./data/www/project-1/.blibox

Then you can copy the templates.

.. code-block:: bash

   host> cp cfg/vhost-gen/*.yml-example-* ./data/www/project-1/.blibox
...
``/home/user/blibox/data/www/project-1/.blibox/apache22.yml``.
``/home/user/blibox/data/www/project-1/.blibox/apache24.yml``,
``/home/user/blibox/data/www/project-1/.blibox/nginx.yml`` or do it for all of them
synchronously.
...
you need to restart the Blibox.
...
.. warning::
   ...
   https://github.com/cytopia/blibox/issues/142
...
.. code-block:: bash
   ...
   blibox@php-7.0.19 in /shared/httpd $ cd project-1/.blibox

   # Check the syntax of apache22.yml
   blibox@php-7.0.19 in /shared/httpd/project-1/.blibox $ yamllint apache22.yml
...
itself or are elsewhere replaced by the Blibox webserver container such as:
...
The Blibox always uses a normal vhost by default, so the ``__VHOST_DOCROOT__`` variable will be
...
The ``vhost_type:`` contains ``docroot`` and ``rproxy``. The Blibox only makes use of ``docroot``
...
.. code-block:: bash
   ...
   host> /home/user/blibox/data/www
...
.. note::
   ...
   rename all project directories. In this case it is much faster to just restart the Blibox.


Restart the Blibox
--------------------

Stop the Blibox and start it up again.
