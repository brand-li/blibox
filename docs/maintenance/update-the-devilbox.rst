...
you actually want to update the Blibox.
...
.. important::
   ...
   with the bundled ``env-example`` file. Different Blibox releases might require different
...
.. _update_the_blibox_update_the_docker_images:
...
For your convenience there is a shell script in the Blibox git directory: ``update-docker.sh``
which will update all available Docker images at once for every version.

.. note::

     The Blibox own Docker images (Apache, Nginx, PHP and MySQL) are even built every night to ensure
...
.. code-block:: bash
   ...
   image: blibox/php-fpm:${PHP_SERVER}-work
   image: blibox/${HTTPD_SERVER}:0.13
   image: blibox/mysql-${MYSQL_SERVER}
...
.. code-block:: bash
   ...
   host> docker pull blibox/php-fpm:7.2-work
...
.. code-block:: bash
   ...
   Pulling php (blibox/php-fpm:5.6-work)...
   Pulling httpd (blibox/apache-2.2:0.13)...
...
In case you also want to pull/update every single of every available Blibox image, you can
...
2. Ensure ``docker-compose rm -f`` is executed after stopping the Blibox
