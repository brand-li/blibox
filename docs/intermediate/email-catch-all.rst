...
all outgoing mails and puts them into a local mailbox by the user ``blibox``.

In order to view sent emails open up the blibox intranet http://localhost/mail.php.
...
have been caught by the Blibox and are available to be read.


MailHog
=======

Instead of using the very basic Blibox intranet UI for emails, you can also enable MailHog
