...
Another core feature of the Blibox, is to be totally independent of what you have or have not
installed on your host operating system.

The Blibox already ships with many common developer tools which are installed inside each PHP
...
When you enter the container with the provided scripts, you are doing so as the user ``blibox``.
If you do need to perform any actions as root (such as installing new software), you can use
the password-less ``sudo``.

.. code-block:: bash

   # Inside the PHP Linux container as user blibox
...
.. seealso:: :ref:`update_the_blibox_update_the_docker_images`
