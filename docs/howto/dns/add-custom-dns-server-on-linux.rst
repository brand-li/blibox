...
an additional entry, which is the one from the Blibox (``127.0.0.1``).
...
If the Blibox DNS server is not running, it does not affect the name resolution, because you will
