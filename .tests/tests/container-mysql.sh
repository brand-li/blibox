...
# Install pipe viewer
run "docker-compose exec --user root     -T php bash -c 'apt update && apt install -y pv'" "${RETRIES}" "${BLIBOX_PATH}"

# Drop database
run "docker-compose exec --user blibox -T php bash -c 'mysql --host=mysql --user=root --password='\\''${MYSQL_ROOT_PASSWORD}'\\'' -e '\\''DROP DATABASE IF EXISTS ${DB_NAME};'\\'''" "${RETRIES}" "${BLIBOX_PATH}"

# Delete mysql.sql file
run "docker-compose exec --user blibox -T php bash -c 'rm -f /home/blibox/mysql.sql'" "${RETRIES}" "${BLIBOX_PATH}"
...
) > /home/blibox/mysql.sql
'" "${RETRIES}" "${BLIBOX_PATH}"
printf "\\n"
run "docker-compose exec --user blibox -T php bash -c 'ls -lap /home/blibox/mysql.sql'" "${RETRIES}" "${BLIBOX_PATH}"

# Import SQL file
run "docker-compose exec --user blibox -T php bash -c 'pv -f -i 1 -p -t -e /home/blibox/mysql.sql | mysql --host=mysql --user=root --password='\\''${MYSQL_ROOT_PASSWORD}'\\'''" "${RETRIES}" "${BLIBOX_PATH}"

# Compare inserted rows
COUNT="$( run "docker-compose exec --user blibox -T php bash -c 'mysql --host=mysql --user=root --password='\\''${MYSQL_ROOT_PASSWORD}'\\'' -e '\\''SELECT COUNT(*) AS cnt FROM ${DB_NAME}.${TBL_NAME};'\\''' | grep -Ei '[0-9]+'" "1" "${BLIBOX_PATH}" )"
