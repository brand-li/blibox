...
.. important::
   When using :ref:`howto_docker_toolbox_and_the_blibox` on Windows, ``*.cnf`` files must have read-only file
...
.. code-block:: bash
   ...
   host> ls -l path/to/blibox/cfg/ | grep -E 'mysql|mariadb|percona'
...
``blibox-custom.cnf-example``, that can simply be renamed to ``blibox-custom.cnf``.
This file holds some example values that can be adjusted or commented out.

In order for the changes to be applied, you will have to restart the Blibox.
...
In order to apply the changes you need to restart the Blibox.
You can validate that the changes have taken place by visiting the Blibox intranet MySQL info page.
