...
settings applied to the Blibox.
...
By default, vhost-gen templates are located within the Blibox root directory under ``cfg/vhost-gen/``.
The templates file names are suffixed with ``-example-<type>`` and are absolutely identical to what is
shipped inside each Blibox web server Docker container.
...
.. |vhost_type| replace:: The vhost type determines the type of vhost: reverse proxy or document root based |br| vhost. The Blibox currently does not support reverse proxy vhost.
