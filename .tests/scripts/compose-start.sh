...
### Startup
###
run "docker-compose up -d" "1" "${BLIBOX_PATH}"
...
until curl -sS -I --fail -o /dev/null -w "%{http_code}" "http://localhost:${HOST_PORT_HTTPD}" 2>/dev/null | grep '200' >/dev/null; do
	...
	if [ "${i}" -eq "60" ]; then
		...
		run "docker-compose exec -T --user blibox php curl -sS -v 'http://localhost/'" "1" "${BLIBOX_PATH}" || true
		run "docker-compose exec -T --user blibox php curl -sS -I 'http://localhost/'" "1" "${BLIBOX_PATH}" || true
		run "docker-compose exec -T --user blibox php curl -sS -I -o /dev/null -w '%{http_code}' 'http://localhost/'" "1" "${BLIBOX_PATH}" || true
		exit 1
	fi
...
i=0
until cd "${BLIBOX_PATH}" && docker-compose exec -T php mongofiles --host=mongo list >/dev/null 2>&1; do
	printf "."

	i=$(( i + 1 ))
	if [ "${i}" -eq "60" ]; then
		printf "[FAIL]\\n"
		cd "${BLIBOX_PATH}" && docker-compose exec -T php mongofiles --host=mongo list
		exit 1
	fi
...
i=0
until cd "${BLIBOX_PATH}" && docker-compose exec -T php mysql --user=root --password="${MYSQL_ROOT_PASSWORD}" --host=mysql -e 'show databases;' 2>&1 | grep mysql >/dev/null; do
	printf "."

	i=$(( i + 1 ))
	if [ "${i}" -eq "60" ]; then
		printf "[FAIL]\\n"
		cd "${BLIBOX_PATH}" && docker-compose exec -T php mysql --user=root --password="${MYSQL_ROOT_PASSWORD}" --host=mysql -e 'show databases;'
		exit 1
	fi
...
i=0
until cd "${BLIBOX_PATH}" && docker-compose exec -T php pg_isready --host=pgsql >/dev/null 2>&1; do
	printf "."

	i=$(( i + 1 ))
	if [ "${i}" -eq "60" ]; then
		printf "[FAIL]\\n"
		cd "${BLIBOX_PATH}" && docker-compose exec -T php pg_isready --host=pgsql
		exit 1
	fi
